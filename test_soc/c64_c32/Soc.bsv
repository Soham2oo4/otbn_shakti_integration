...
`endif
  ...
  `ifdef debug
    interface AXI4_Slave_IFC#(`paddr, `buswidth, USERSPACE) to_debug_master;
    interface AXI4_Master_IFC#(`paddr, `buswidth, USERSPACE) to_debug_slave;
  ...
  module mkSoc `ifdef debug #(Vector#(`num_harts, Reset) hartresets ) `endif (Ifc_Soc);
    ...
    AXI4_Fabric_IFC #(Num_Masters, `Num_Slaves, `paddr, `buswidth, USERSPACE) 
    ...
    Ifc_sign_dump signature<- mksign_dump();
	  Ifc_uart_axi4#(`paddr,`buswidth,0, 16) uart <- mkuart_axi4(curr_clk,curr_reset, 5, 0, 0);
    Ifc_clint_axi4#(`paddr, `buswidth, 0, `num_harts, 2) clint <- mkclint_axi4();
    Ifc_err_slave_axi4#(`paddr,`buswidth,0) err_slave <- mkerr_slave_axi4;
    Ifc_bram_axi4#(`paddr, `buswidth, USERSPACE, `Addr_space) main_memory <- mkbram_axi4(`MemoryBase,
                                                "code.mem", "MainMEM");
		Ifc_bootrom_axi4#(`paddr, `buswidth, USERSPACE, 12) bootrom <-mkbootrom_axi4(`BootRomBase);
