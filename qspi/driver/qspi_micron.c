...
//Read -  Quad mode - SDR
    ...
    ar_read=0;
    for(i=0;i<WRITEB;i+=4){
        flashReadQuadSPI(5,ar_read,0xEB,16,FOURBYTE);
        waitfor(100);
        ar_read+=16;
    }
    //return 0;
...
//Uncomment one of the following for the required XIP mode operation

//  flashSingleSPIXip(0x000000, config_string);   //Single mode SDR
//	flashSingleSPIDDRXip(0x000000, config_string);//Single mode DDR
//	flashDualSPIXip(0x000000, config_string);     //Dual mode SDR
//	flashDualSPIDDRXip(0x000000, config_string);  //Dual mode DDR
//	flashQuadSPIXip(0x000000, config_string);     //Quad mode SDR
//	flashQuadSPIDDRXip(0x000000, config_string);  //Quad mode DDR
