...
from soc_config.utils import yaml
import soc_config.utils as utils
from soc_config.errors import ValidationError
from soc_config.consts import *
import os
import logging
import sys
import math
...
def capture_compile_cmd(foo, isa_node, debug_spec, grouping_spec):
    ...
    if 'S' in isa_node['ISA']:
        ...
        asidlen = 0
        asid_node = isa_node['satp']['rv' + str(xlen)]['asid']
        import re
        legal_str = asid_node['type']['warl']['legal'][0]
        match = re.search(r'asid\[(\d+):(\d+)\]', legal_str)
        if match:
            msb = int(match.group(1))
            lsb = int(match.group(2))
            asidlen = msb - lsb + 1
        else:
            asidlen = 0  # fallback if pattern not found
 ...
 G0010:T0054:G0020:G0024:G0023:G0096:G0036:G0117:G0015'
    ...
    if debug_spec is not None:
        macros += ' debug'
        macros += ' debug_bus_sz='+str(foo['bus_protocol_configuration']['bus_width'])

    if foo['bsc_compile_options']['assertions']:
        macros += ' ASSERT'
    if foo['bsc_compile_options']['trace_dump']:
        macros += ' rtldump'
    if foo['bsc_compile_options']['etrace_port']:
        macros += ' etrace_support'
    if foo['bsc_compile_options']['ovl_assertions']:
        macros += ' ovl_assert'
    ...
    macros += ' RV'+str(xlen)+' ibuswidth='+str(foo['bus_protocol_configuration']['bus_width'])
    macros += ' dbuswidth='+str(foo['bus_protocol_configuration']['bus_width'])
    macros += ' resetpc='+str(foo['reset_pc'])
    macros += ' buswidth='+str(foo['bus_protocol_configuration']['bus_width'])
    macros += ' axi4_transactor_fifo_depth='+str(foo['bus_protocol_configuration']['axi4_transactor_fifo_depth'])
    macros += ' axi4_internal_fifo_depth='+str(foo['bus_protocol_configuration']['axi4_internal_fifo_depth'])
    macros += ' axi4_lite_transactor_fifo_depth='+str(foo['bus_protocol_configuration']['axi4_lite_transactor_fifo_depth'])
    macros += ' axi4_lite_internal_fifo_depth='+str(foo['bus_protocol_configuration']['axi4_lite_internal_fifo_depth'])
    macros += ' axi4_id_width='+str(foo['bus_protocol_configuration']['axi4_id_width'])
    macros += ' USERSPACE='+str(foo['bus_protocol_configuration']['userspace_width'])
    if foo['bus_protocol_configuration']['bus_width'] == 128:
        macros += ' axi4_128b'
        
    macros += ' paddr='+str(isa_node['physical_addr_sz'])
    macros += ' vaddr='+str(xlen)
    macros += ' CORE_'+str(foo['bus_protocol_configuration']['bus_protocol'])
    ...
    if foo.get('hardfloat', False):
        macros += ' hardfloat'
    if foo.get('fpu_clockgate', False):
        macros += ' common_bsv_TX_relaxed fpu_clockgate'
...
def generate_makefile(foo, logging=False):
    ...
    bsv_path_file = open(f'{top_dir}/bsvpath','r').read().splitlines()
...
def validate_specs(uarch_spec, isa_spec, debug_spec, grouping_spec, logging=False):
    ...
    schema = os.path.join(package_dir, 'schema.yaml')
