...
'''
    ...
    for csrname in isa_yaml:
        if attr_yaml[csrname]['value_method']['required']:
            bsvstr += '''
        `{0} : return zeroExtend({2}.soc_sb.sbread.mv_csr_{1});'''.format(csrname.upper(),
...
def csr_decoder(isa_yaml, work_dir, bsv, logging=False):
    ...
    if 'F' in isa_yaml['hart0']['ISA'] :
        definefile.write(func_decoder_fd)
    else :
    ...
    for node in isa_yaml['hart0']:
        ...
        if isinstance(isa_yaml['hart0'][node], dict):
            if 'address' in isa_yaml['hart0'][node] and ( isa_yaml['hart0'][node][rvxlen]['accessible'] == True) :
                ...
                if ((node[0]=='f' or node[0]=='F') and 'F' in isa_yaml['hart0']['ISA']):
                    cond_str += 'if (fs!=0)' 
        ...
        for m in regex_ass_legal.finditer(assignstr):
            ...
            if op == 'in':
                ...
                for v in values:
                    ...
                    # ignore checks
                    elif base == 0 and bound == (2**(msb+1))-1:
                        singlestr += 'True'
                    # range value checks
                    else:
...
def station_module(isa_yaml, grp_yaml, attr_yaml, rvxlen, pkg_name,
    ...
    param_str = ''
    for csrname in isa_yaml:
        if 'mod_parameters' in attr_yaml[csrname]:
            if param_str == '':
                param_str = "#("
            else :
                param_str = param_str[:-1]+","

            param_str += " parameter "+ attr_yaml[csrname]['mod_parameters']+" )"
    ...
    bsvstr += (module_temp.format(pkg_name, attr_str, param_str))
    if logging:
...
def csr_top(isa_yaml, grp_yaml, attr_yaml, aux_info, rvxlen, pkgname, logging = False):
  ...
  endinterface:Sbread'''
    ...
    perf_grp_list = []
    for v in counters_list[2:]:
        n = find_group(grp_yaml, v)
        if n is not None:
            perf_grp_list.append((int(n[3:]),v))
    ...
    for (n,v) in perf_grp_list:
        if n >= 2:
            mod_attr_str += '\n(*preempts="x{0}_mkConnectionAVtoAf,grp{1}_rl_increment_{2}"*)'.format(str(n-2),str(n),v.replace("ounter",""))

    extra_reset = "#(parameter Bit#({0}) hartid)".format(str(xlen))
    ...
    for grps in grp_yaml:
        if grps == "grp1":
            bsvstr += '    let ' + grps + ' <- mk_csrbox_' + grps + '(hartid);\n'
