...
module mkMIMOReg#(MIMOConfiguration cfg)(MIMO#(max_in, max_out, size, t))
   ...
   rule update;
      ...
      shift_amt = cExtend(nextDataAvail) * fromInteger(valueOf(st)) - cExtend(enqCount) * fromInteger(valueOf(st));
      ...
      /* $display("enqCount      = %h", enqCount );  
       $display("shift_amt     = %d", shift_amt );
       $display("enqData       = %h", enqData );
       $display("enqMask       = %h", enqMask );
       $display("nextStorage   = %h", nextStorage );    
       $display("nextMask      = %h", nextMask );  
       $display("nextDataCount = %d", nextDataCount ); 
       //$display("lastpointer   = %d",   lastpointer ); 
       $display("nextDataAvail = %d", nextDataAvail ); 
       $display("rStorage      = %h", rStorage ); 
       $display("UB            = %d", valueOf(total)-1 ); 
       $display("LB            = %d", valueOf(total)-valueOf(outtot) );*/ 
   ...
   method Vector#(max_out, t) first() if (cfg.unguarded || (rDataCount > 0));
      Bit#(outtot) result = rStorage[valueOf(total)-1:valueOf(total)-valueOf(outtot)]; 
...
/*module tbmktrace();      
	...
	MIMO#(32,4,64,Bit#(8)) trace_sink_buffer <- mkMIMO(cfg);
      
           
            rule step1(rg_state == 0);
            Vector#(32, Bit#(8)) lv_payload_1 = newVector();
                      lv_payload_1[0] = 8'h1f; 
                trace_sink_buffer.enq(6'h1,lv_payload_1); 
                 $display("enque1 done"); 
                 $display("\n");
                 rg_state <= 1 ;
                  $display("trace_sink_buffer.first = %h", trace_sink_buffer.first );  
            endrule
            
            rule step2(rg_state == 1);
            Vector#(32, Bit#(8)) lv_payload_1 = newVector();
                      lv_payload_1[6] = 8'h73;
                      lv_payload_1[5] = 8'h00; 
                      lv_payload_1[4] = 8'h00; 
                      lv_payload_1[3] = 8'h00; 
                      lv_payload_1[2] = 8'h00; 
                      lv_payload_1[1] = 8'h00; 
                      lv_payload_1[0] = 8'h04;  
                 trace_sink_buffer.enq(6'h7,lv_payload_1);
                  $display("enque2 done"); 
                 $display("\n");
                 rg_state <= 2 ; 
                  $display("trace_sink_buffer.first = %h", trace_sink_buffer.first ); 
            ...
            Vector#(32, Bit#(8)) lv_payload_1 = newVector();
                      lv_payload_1[5] = 8'haa;
                      lv_payload_1[4] = 8'hbb; 
                      lv_payload_1[3] = 8'hcc; 
                      lv_payload_1[2] = 8'hdd; 
                      lv_payload_1[1] = 8'hee; 
                      lv_payload_1[0] = 8'hff;  
                 trace_sink_buffer.enq(6'h6,lv_payload_1);
                  $display("enque3 done"); 
                 $display("\n");
                 rg_state <= 3 ; 
                  $display("trace_sink_buffer.first = %h", trace_sink_buffer.first ); 
            ...
            rule step4(rg_state == 3);
                  ...
                  $finish(0); end
                     ...
                     $display("trace_sink_buffer.first = %h", trace_sink_buffer.first ); 
                      trace_sink_buffer.deq(4);
                    
                 count<= count + 1;
            endrule
     endmodule*/

endpackage: MIMO_TRACE_MODIFY
