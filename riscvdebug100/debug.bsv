...
`ifndef debugrom_large  // normal debug ROM
  ...
  `ifndef iclass // other cores
    vrom[0] = 'h00c0006f; //<entry>: 0x800:	00c0006f          	j	c <_entry>
    vrom[1] = 'h0600006f; //<resume>: 0x804:	0600006f          	j	64 <_resume>
    vrom[2] = 'h0380006f; //<exception>: 0x808:	0380006f          	j	40 <_exception>
    vrom[3] = 'h0ff0000f; //<_entry>: 0x80c:	0ff0000f          	fence  
    vrom[4] = 'h7b241073; //0x810:	7b241073          	csrw	dscratch0,s0 
    vrom[5] = 'hf1402473; //<entry_loop>: 0x814:	f1402473          	csrr	s0,mhartid
    vrom[6] = 'h10802023; // 0x818:	10802023          	sw	s0,256(zero) # 100 <_resume+0x9c>
    vrom[7] = 'h40044403; // 0x81c:	40044403          	lbu	s0,1024(s0)
    vrom[8] = 'h00147413; // 0x820:	00147413          	andi	s0,s0,1
    vrom[9] = 'h02041463; // 0x824:	02041463          	bnez	s0,4c <going>
    vrom[10] = 'hf1402473; //0x828:	f1402473          	csrr	s0,mhartid
    vrom[11] = 'h40044403; //0x82c:	40044403          	lbu	s0,1024(s0)
    vrom[12] = 'h00247413; //0x830:	00247413          	andi	s0,s0,2
    vrom[13] = 'h02041863; //0x834:	02041863          	bnez	s0,64 <_resume>
    vrom[14] = 'h00000013; //0x838:	00000013          	nop
    vrom[15] = 'hfd9ff06f; //0x83c:	fd9ff06f          	j	14 <entry_loop>
    vrom[16] = 'h7b202473; //<_exception>: 0x840:	7b202473          	csrr	s0,dscratch0
    vrom[17] = 'h10002623; //0x844:	10002623          	sw	zero,268(zero) # 10c <_resume+0xa8>
    vrom[18] = 'h00100073; //0x848:	00100073          	ebreak
    vrom[19] = 'hf1402473; //<going>: 0x84c:	f1402473          	csrr	s0,mhartid
    vrom[20] = 'h10802223; //0x850:	10802223          	sw	s0,260(zero) # 104 <_resume+0xa0>
    vrom[21] = 'h7b202473; //0x854:	7b202473          	csrr	s0,dscratch0
    vrom[22] = 'h0ff0000f; //0x858:	0ff0000f          	fence
    vrom[23] = 'h0000100f; //0x85c:	0000100f          	fence.i
    vrom[24] = 'h30000067; //0x860:	30000067          	jr	zero # 0 <entry>
    vrom[25] = 'hf1402473; //<_resume>: 0x864:	f1402473          	csrr	s0,mhartid
...
`endif
  ...
  rule rl_upd_flags;
    ...
    if (wr_cmdtype_wren && wr_cmdtype_wrval == 0) // go abstract command
      lv_go = 1;
    else if (wr_hartgoing_wren) // this wire is set when GOING is written by hart.
      lv_go = 0;
    for (Integer i = 0; i<v_ncomponents; i = i + 1) begin
  ...
  rule rl_sba_read_response(sbbusy==1 && !rg_sbwrite_en && !rg_sbread_en);
    ...
    else if (response.rresp == AXI4_SLVERR) begin
      sberr <= pack(SbOther);
    end
    else begin
  ...
  rule rl_bus_read;
    ...
    if (offset == `IMPEBREAK) begin // reading implicit ebreak
      ...
      // TODO: add support for 8-byte read if required
      `ifdef axi4_128b
