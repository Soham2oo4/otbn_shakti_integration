...
`endif
  ...
  `else
    AXI4_Slave_Xactor_IFC#(`paddr, `id_width, `debug_bus_sz, `USERSPACE) slave_xactor <- mkAXI4_Slave_Xactor;//(reset_by dm_reset);
    AXI4_Master_Xactor_IFC#(`paddr, `id_width, `debug_bus_sz, `USERSPACE) master_xactor <- mkAXI4_Master_Xactor(reset_by dm_reset);
  `endif
...
`endif
  ...
  rule rl_sba_request( (rg_sbread_en || rg_sbwrite_en) && sbbusy == 1);
    ...
    if (lv_err == SbSuccess) begin
      AXI4_Rd_Addr#(`paddr, `id_width, `ifndef iclass 0 `else `USERSPACE `endif ) read_request = AXI4_Rd_Addr{araddr: truncate(address),aruser: 0, 
                                      arlen : 0, arsize: sbaccess, arburst: 0,
                                      arid  : 0, arprot:'d3};
      AXI4_Wr_Addr#(`paddr, `id_width, `ifndef iclass 0 `else `USERSPACE `endif ) wr_addr_request = AXI4_Wr_Addr{awaddr: truncate(address),awuser: 0, 
                                      awlen : 0, awsize: sbaccess, awburst: 0,
                                      awid  : 0, awprot:'d3};
      AXI4_Wr_Data#(`id_width, `debug_bus_sz) wr_data_request = AXI4_Wr_Data{ wdata: writedata, wstrb: writestrb, wlast: True};
  ...
  rule rl_bus_read;
    ...
    `endif
      ...
      succ = False;
	 	...
	 	AXI4_Rd_Data#(`id_width, `debug_bus_sz, `ifndef iclass 0 `else `USERSPACE `endif ) r = AXI4_Rd_Data {rresp: succ?AXI4_OKAY:AXI4_SLVERR,rid:req.arid,rlast:(req.arlen==0), 
          rdata: data, ruser: 0};
