// Verilated -*- C++ -*-
// DESCRIPTION: Verilator output: Design implementation internals
// See Vimem_test.h for the primary calling header

#include "Vimem_test__pch.h"
#include "Vimem_test_prim_sha2_pkg.h"

VL_ATTR_COLD void Vimem_test_prim_sha2_pkg___ctor_var_reset(Vimem_test_prim_sha2_pkg* vlSelf) {
    (void)vlSelf;  // Prevent unused variable warning
    Vimem_test__Syms* const __restrict vlSymsp VL_ATTR_UNUSED = vlSelf->vlSymsp;
    VL_DEBUG_IF(VL_DBG_MSGF("+              Vimem_test_prim_sha2_pkg___ctor_var_reset\n"); );
}
