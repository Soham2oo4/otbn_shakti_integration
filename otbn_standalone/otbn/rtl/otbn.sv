...
#(
  ...
  logic [63:0] imem_wdata_bus;
  logic [63:0] imem_wmask_bus;
  logic [38:0] imem_wdata_bus_secded; // ECC-encoded instruction for IMEM write
  logic [63:0] imem_rdata_bus;
  logic [38:0] imem_rdata_bus_raw;
  ...
  // Compute proper inverted-SECDED (prim_secded_inv_39_32) ECC for IMEM bus writes.
  // Zero ECC bits would cause uncorrectable errors when the core fetches instructions.
  prim_secded_inv_39_32_enc u_imem_wdata_enc (
    .data_i(imem_wdata_bus[31:0]),
    .data_o(imem_wdata_bus_secded)
  );

  assign imem_wdata = imem_access_core ? '0 : imem_wdata_bus_secded;
  ...
  assign imem_wmask = imem_access_core ? '1 : {{7{|imem_wmask_bus[31:0]}}, imem_wmask_bus[31:0]};
  `ASSERT(ImemWmaskBusIsFullWord_A, imem_req_bus && imem_write_bus |-> imem_wmask_bus[31:0] == '1)
  ...
  // With EnableDataIntgPt=0, adapter handles integrity at bus boundary.
  // Just pass raw 32-bit data zero-extended to 64-bit.
  assign imem_rdata_bus = {32'h0, imem_rdata_bus_raw[31:0]};
  ...
  logic [ExtWLEN-1:0] dmem_rdata_bus_raw;
  logic [ExtWLEN-1:0] dmem_wdata_bus_enc;
  logic [ExtWLEN-1:0] dmem_wmask_bus_enc;
  ...
  // Encode 256-bit raw bus data to 312-bit ECC'd format for SRAM
  for (genvar i = 0; i < BaseWordsPerWLEN; i++) begin : g_dmem_bus_ecc_enc
    prim_secded_inv_39_32_enc u_dmem_bus_enc (
      .data_i(dmem_wdata_bus[i*32+:32]),
      .data_o(dmem_wdata_bus_enc[i*39+:39])
    );
    assign dmem_wmask_bus_enc[i*39+:39] = {
      {7{|dmem_wmask_bus[i*32+:32]}},
      dmem_wmask_bus[i*32+:32]
    };
  end
  ...
  assign dmem_wmask = dmem_access_core ? dmem_wmask_core : dmem_wmask_bus_enc;
  // SEC_CM: DATA.MEM.SW_NOACCESS
  assign dmem_index = dmem_access_core ? dmem_index_core : dmem_index_bus;
  assign dmem_wdata = dmem_access_core ? dmem_wdata_core : dmem_wdata_bus_enc;
  ...
  // is asserted the cycle the rdata is being returned no locking was occurring when the request
  // came in so it is reasonable to proceed with returning the supplied integrity.
  // With EnableDataIntgPt=0, strip ECC and pack raw 32-bit data for the adapter
  for (genvar i_word = 0; i_word < BaseWordsPerWLEN; ++i_word) begin : g_dmem_rdata_bus
    assign dmem_rdata_bus[i_word*32+:32] = dmem_rdata_bus_raw[i_word*39+:32];
  end
  ...
  // `ASSERT_INIT(TLDWIs32Bit_A, top_pkg::TL_DW == 32)  // Disabled for TL64

  // Only advance CRC calculation on full writes;
  assign mem_crc_data_in_valid   = ~(dmem_access_core | imem_access_core) &
      ((imem_req_bus & (imem_byte_mask_bus == '1)) |
       (dmem_req_bus & (dmem_byte_mask_bus == '1)));
