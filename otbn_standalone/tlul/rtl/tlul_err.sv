...
module tlul_err import tlul_pkg::*; (
    ...
    if (tl_i.a_valid) begin
      unique case (tl_i.a_size)
        ...
        'h1: begin // 2 Byte
          ...
          // check inactive lanes for the selected 2-byte lane
        ...
        'h2: begin // 4 Byte
          addr_sz_chk  = ~|tl_i.a_address[1:0];
          // check inactive lanes for the selected 4-byte lane
          mask_chk     = (tl_i.a_address[2]) ? ~|(tl_i.a_mask & 8'b00001111)
                       : ~|(tl_i.a_mask & 8'b11110000);
          fulldata_chk = (tl_i.a_address[2]) ? &tl_i.a_mask[7:4] : &tl_i.a_mask[3:0];
        end

        'h3: begin // 8 Byte (full TL64 word)
  ...
  // Only 64 bit data width for current tlul_err (TL64)
  `ASSERT_INIT(dataWidthOnly64_A, DW == 64)
