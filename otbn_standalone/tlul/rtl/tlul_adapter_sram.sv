...
#(
  ...
  // For TL64: full-word size is $clog2(TL_DBW) = 3, not 2.
  localparam int unsigned TlFullWordSize = $clog2(top_pkg::TL_DBW);
  assign wr_attr_error = (ByteAccess == 0) &&
                         (tl_i.a_opcode == PutFullData || tl_i.a_opcode == PutPartialData) &&
                         (tl_i.a_mask != '1 ||
                          tl_i.a_size != top_pkg::TL_SZW'(TlFullWordSize));
  ...
  assign data_intg = (reqfifo_rdata.error) ? error_blanking_integ    : // TL-UL error
                     ...
                     prim_secded_pkg::SecdedInv7264ZeroEcc;            // valid write
  ...
  if (EnableDataIntgPt) begin : gen_no_rmask
    always_comb begin
      ...
      rdata_tlword = prim_secded_pkg::SecdedInv7264ZeroWord;
