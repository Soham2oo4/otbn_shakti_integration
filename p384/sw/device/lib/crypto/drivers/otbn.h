...
/**
 * Generate the name of the compile-time constant for an OTBN remote symbol.
 *
 * On RV64 (Shakti) remote symbols are #defined constants from the generated
 * header (gen/run_p384_defs.h) rather than linker symbols.  The constant name
 * follows the _OTBN_REM_<app>_<sym> convention.
 */
#define OTBN_SYMBOL_ADDR(app_name, sym) _OTBN_REM_##app_name##_##sym
 ...
 *
 * On RV64 (Shakti) these symbols are compile-time #defines from the generated
 * header (included via -include gen/run_p384_defs.h).  No extern declaration
 * is needed; this macro is a no-op so that OTBN_ADDR_T_INIT can resolve the
 * #defined constant directly.
 */
#define OTBN_DECLARE_SYMBOL_ADDR(app_name, symbol_name) /* no-op on RV64 */
 ...
 * On RV64 (Shakti): symbol is a scalar uint32_t, use its value directly.
 */
#define OTBN_ADDR_T_INIT(app_name, symbol_name) \
  (OTBN_SYMBOL_ADDR(app_name, symbol_name))
